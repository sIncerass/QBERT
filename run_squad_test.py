...
def run_squad_w_args(args):
    ...
    if args.do_train:
        ...
        each_step = int(len(train_examples) / args.train_batch_size / 20)
        print(global_step, each_step)
        model.train()
        for _ in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(
                ...
                if (step + 1) % args.gradient_accumulation_steps == 0:
                    ...
                    optimizer.zero_grad()
                    global_step += 1
                if (step+1) % each_step == 0 and args.do_train and (args.local_rank == -1
                          or torch.distributed.get_rank() == 0):
                    # cur_id = (step+1) % each_step
                    # Save a trained model, configuration and tokenizer
                    model_to_save = model.module if hasattr(
                        model, 'module') else model  # Only save the model it-self
                    cur_output_dir = args.output_dir + "/step_" + str(step)
                    if not os.path.exists(cur_output_dir):
                        os.makedirs(cur_output_dir)
                    open(cur_output_dir + "/loss.txt", "w").write( "%s\n" % str(total_loss / (step+1)) )
                    # If we save using the predefined names, we can load using
                    # `from_pretrained`
                    output_model_file = os.path.join(cur_output_dir, WEIGHTS_NAME)
                    output_config_file = os.path.join(cur_output_dir, CONFIG_NAME)

                    torch.save(model_to_save.state_dict(), output_model_file)
                    model_to_save.config.to_json_file(output_config_file)
                    tokenizer.save_vocabulary(cur_output_dir)
