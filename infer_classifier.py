...
class MnliProcessor(DataProcessor):
    ...
    def _create_examples(self, lines, set_type, test=False):
        ...
        for (i, line) in enumerate(lines):
            ...
            text_a = line[8]
            text_b = line[9]
            label = "neutral" if test else line[-1]
            examples.append(
                InputExample(
                    guid=guid, text_a=text_a, text_b=text_b, label=label))
        return examples
...
class QnliProcessor(DataProcessor):
    ...
    def _create_examples(self, lines, set_type, test=False):
        ...
        for (i, line) in enumerate(lines):
            ...
            text_a = line[1]
            text_b = line[2]
            label = "entailment" if test else line[-1]
            examples.append(
                InputExample(
                    guid=guid, text_a=text_a, text_b=text_b, label=label))
        return examples
...
def run_classifier_w_args(args):
    ...
    nb_tr_steps = 0
    tr_loss = 0
    data_dir = "../data/glue_data/" + output_name
    ...
    if args.do_eval and (args.local_rank == -1
        ...
        if output_name == "MNLI":
            output_eval_file = os.path.join(args.output_dir, "MNLI-m.tsv")
        else:
            output_eval_file = os.path.join(args.output_dir, f"{output_name}.tsv")

        with open(output_eval_file, "w") as writer:
            writer.write('index\tprediction\n')
            for cnt, pred in enumerate(preds):
                writer.write( f'{cnt}\t{label_list[1-pred]}\n' )
        ...
        if task_name == "mnli":
            ...
            output_eval_file = os.path.join(args.output_dir, "MNLI-mm.tsv")
            with open(output_eval_file, "w") as writer:
                writer.write('index\tprediction\n')
                for cnt, pred in enumerate(preds):
                    writer.write( f'{cnt}\t{label_list[pred]}\n' )
