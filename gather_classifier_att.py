...
# Only allow for 1 GPU, multiple GPU support is not assured.
def attention_data(model, hidden_states, attention_mask, head_mask=None):
    mixed_query_layer = model.query(hidden_states)
    mixed_key_layer = model.key(hidden_states)
    mixed_value_layer = model.value(hidden_states)

    query_layer = model.transpose_for_scores(mixed_query_layer)
    key_layer = model.transpose_for_scores(mixed_key_layer)
    value_layer = model.transpose_for_scores(mixed_value_layer)

    # Take the dot product between "query" and "key" to get the raw attention scores.
    attention_scores = torch.matmul(query_layer, key_layer.transpose(-1, -2))
    attention_scores = attention_scores / math.sqrt(model.attention_head_size)
    ...
    attention_probs = model.dropout(attention_probs)

    return {'attn': attention_probs, 'queries': query_layer, 'keys': key_layer}
...
def run_classifier_w_args(args):
        ...
        def get_attention_modules(model, prefix=''):
            for name, layer in model._modules.items():
                ...
                if len(layer._modules.items()) > 0:
                    yield from get_attention_modules(
                        layer, prefix=prefix + '.' + name)
...
def get_parser():
    ...
    parser.add_argument(
        "--no_update", action='store_true', help="Whether to update model.")
    parser.add_argument(
        "--do_eval",
        action='store_true',
